...
public class MeasurementActivity extends AppCompatActivity {
    ...
    private class FaceTracker extends Tracker<Face> {
        ...
        public void onUpdate(Detector.Detections<Face> detections, Face face) {
            ...
            float distance;
            if (deltaX >= deltaY) {
                distance = (F * (AVERAGE_EYE_DISTANCE / sensorX) * (IMAGE_WIDTH / deltaX)) / 10;
            } else {
                distance = (F * (AVERAGE_EYE_DISTANCE / sensorY) * (IMAGE_HEIGHT / deltaY)) / 10;
            }

            showStatus("Distance = " + String.format("%.3f", distance) + " cm");
